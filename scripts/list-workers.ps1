[CmdletBinding()]
param([switch]$Json, [string]$OwnerId = "")
$raw = & (Join-Path $PSScriptRoot "list-pairs.ps1") -Json -OwnerId $OwnerId
...
$workers = @($pairs | ForEach-Object {
    [PSCustomObject]@{
        worker_id=$_.pair_id; pair_id=$_.pair_id; name=$_.worker_name; owner_id=$_.owner_id; mode=$_.mode; sandbox=$_.sandbox
